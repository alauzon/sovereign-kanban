...
| `tests/Functional/assignee_validation_enforcement.php` | An assignee must be an **existing Nextcloud account**: a ghost id gets **400 `invalid_assignee`** naming the culprit, one bad entry rejects the whole list (no partial write), valid accounts pass, clearing works, and a file already carrying stale free-text garbage **still loads and still updates** (validation fires on the parameter, never on the past). Guards D5 — "the worst gap" in the Deck→SK correspondence. v1 scope: existing user; participant-of-board comes with the Vue NcSelect. |
| `tests/Functional/date_validation_enforcement.php` | A malformed date is **refused (400 `invalid_date`), not stored**: a 6-digit year (`202607-07-19`), Feb 30, free text — all rejected, nothing written; valid `Y-m-d` and `Y-m-d\TH:i` pass; clearing works. Guards the corruption Steve hit on Chrome/Windows 2026-07-18, where the tolerant `normalizeDate` stored a 6-digit-year date verbatim. Validation lives in the controller; `normalizeDate` stays tolerant so already-corrupt cards still load. |
