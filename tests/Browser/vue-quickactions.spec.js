// Quick actions on a tile (Alain, 2026-07-19): mark done / reopen and delete
...
test.describe('actions rapides sur la tuile (Vue)', () => {
	...
	test('marquer fait puis supprimer depuis la tuile', async ({ page }) => {
		...
		// Mark done from the tile → the card becomes --done.
		await card.hover()
		await card.getByRole('button', { name: 'Marquer comme fait' }).click()
		await expect(page.locator('.sk-vue-card--done', { hasText: CARD })).toHaveCount(1)

		// Reopen from the tile → no longer done.
		await card.hover()
		await card.getByRole('button', { name: 'Rouvrir' }).click()
		await expect(page.locator('.sk-vue-card--done', { hasText: CARD })).toHaveCount(0)

		// Delete from the tile → the card disappears.
		await card.hover()
		await card.getByRole('button', { name: 'Supprimer la carte' }).click()
